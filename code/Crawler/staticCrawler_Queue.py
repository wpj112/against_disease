...
class SpiderModel:
    ...
    def GetPageRecur(self,url):
        ...
        #recursive
        self.getFh21Urls(soup, url)
       # print curPageUrls
        #while not self.urlsQueue.empty():
        #    nextUrl = self.urlsQueue.get()
        #    print str(self.curCnt) + ":will crawl " + nextUrl
        #    self.GetPageRecur(nextUrl)
        #except Exception,e:
        #    print Exception,":",e
    ...
    def getFh21Urls(self, soup, curUrl):
        ...
        #        urls.append(a['href'])
                if self.crawleredDict.has_key(a['href']):
                    continue
                self.urlsQueue.put(a['href'])
            else:
                #urls.append(self.fhPrefix + a['href'])
                if self.crawleredDict.has_key(self.fhPrefix + a['href']):
                    continue
                self.urlsQueue.put(self.fhPrefix + a['href'])
        #print a['href']
        print "cur Queue size:%d" % (self.urlsQueue.qsize())
...
#                    curPageContent += curP.get_text()+"++"
    ...
    def mainCrawer(self, enterUrl):
        self.urlsQueue.put(enterUrl)
        while not self.urlsQueue.empty():
            nextUrl = self.urlsQueue.get()
            print str(self.curCnt) + ":will crawl " + nextUrl
            self.GetPageRecur(nextUrl)
...
spriderHander.mainCrawer("http://wap.fh21.com.cn/view/764718.html")
